...
/*
Элемент управления WebBrowser control для .NET это лишь обмотка IE ActiveX control.
Но она не выставляет все события, которые есть у элемента управления IE ActiveX.

Например, у ActiveX контрола есть метод NewWindow2, который можно использовать для перехвата,
когда создаётся новое окно, и даже можно использовать переменную ppDisp,
чтобы задать указатель на экземпляр IE ActiveX, где будет отображаться новое окно.
...
Look at the IE Activex info and add the  DWebBrowserEvents2 and IWebBrowser2 COM interfaces.
We need them to make our hooks.
Create a WebBrowserExtendedEvents extending System.Runtime.InteropServices.StandardOleMarshalObject and DWebBrowserEvents2.
We need this class to intercept the ActiveX events. Add methos for all the events that you want to intercept.
Extend the WebBrowser control overriding the CreateSink and DetachSink methods, 
here is where the WebBrowserExtendedEvents class is used to make the connection.
